...
$qry->select('manifest_cache')
	->from('#__extensions')
	->where('type = ' . $qry->quote('component'))
	->where('element = ' . $qry->quote('com_virtuemart'));

$dbo->setQuery($qry);

$manifest = json_decode($dbo->loadResult());

if ($manifest->version == '3.0.18')
{
	return require_once dirname(__FILE__) . '/vmview_v3.0.18.php';
}
...
class VmView extends JViewLegacy
{
	...
	static public function getVmSubLayoutPath($name)
	{
		...
		$locations = array(
			...
			JPATH_THEMES . "/{$template}/niches/{$niche}/html/com_virtuemart/sublayouts",
			
			// Template override directory in active template folder.
			JPATH_THEMES . "/{$template}/html/com_virtuemart/sublayouts",
			
			// Sun Framework's template override directory.
			SUNFW_PATH . '/includes/overwrite/html/com_virtuemart/sublayouts'
		);
		
		foreach ($locations as $location)
		{
			$templatePath = "{$location}/{$name}.php";
		...
		elseif (JFile::exists(VMPATH_SITE . DS . 'sublayouts' . DS . $name . '.php'))
		{
	...
	function prepareContinueLink()
	{
		...
		else
		{
			...
			$uri = $juri->toString(array(
				'host',
				'port'
			));
			
			$scheme = $juri->toString(array(
				'scheme'
			));
		...
		$this->continue_link_html = '<a class="continue_link" href="' . $this->continue_link . '">' . vmText::_('COM_VIRTUEMART_CONTINUE_SHOPPING') .
			 '</a>';
	...
	function linkIcon($link, $altText = '', $boutonName, $verifyConfigValue = false, $modal = true, $use_icon = true, $use_text = false, $class = '')
	{
		...
		$text = '';
		if ($use_icon)
			$text .= JHtml::_('image', $folder . $boutonName . '.png', vmText::_($altText), null, false, false); //$folder shouldn't be as alt text, here it is: image(string $file, string $alt, mixed $attribs = null, boolean $relative = false, mixed $path_rel = false) : string, you should change first false to true if images are in templates media folder
		if ($use_text)
			$text .= '&nbsp;' . vmText::_($altText);
		if ($text == '')
			$text .= '&nbsp;' . vmText::_($altText);
		if ($modal)
			return '<a ' . $class . ' class="modal" rel="{handler: \'iframe\', size: {x: 700, y: 550}}" title="' . vmText::_($altText) . '" href="' .
				 JRoute::_($link, FALSE) . '">' . $text . '</a>';
		else

...
class JViewLegacy extends JObject
{
	...
	public function __construct($config = array())
	{
		...
		$this->component = JApplicationHelper::getComponentName();
		$this->sunfw_template = JFactory::getApplication()->getTemplate();
		$this->niche = SunFwHelper::getActiveNicheStyle();
	...
	public function loadTemplate($tpl = null)
	{
		...
		$lang->load('tpl_' . $this->sunfw_template, JPATH_BASE, null, false, true) ||
			 $lang->load('tpl_' . $this->sunfw_template, JPATH_THEMES . "/$this->sunfw_template", null, false, true);
		
		// Change the template folder if alternative layout is in different template
		if (isset($layoutTemplate) && $layoutTemplate != '_' && $layoutTemplate != $this->sunfw_template)
		{
			$this->_path['template'] = str_replace($this->sunfw_template, $layoutTemplate, $this->_path['template']);
		}
		
		// Load the template script
		jimport('joomla.filesystem.path');
		$filetofind = $this->_createFileName('template', array(
			'name' => $file
		));
		...
		{
			$filetofind = $this->_createFileName('', array(
				'name' => 'default' . ( isset($tpl) ? '_' . $tpl : $tpl )
			));
			$this->_template = JPath::find($this->_path['template'], $filetofind);
		}
	...
	public function loadHelper($hlp = null)
	{
		...
		// Load the template script
		jimport('joomla.filesystem.path');
		$helper = JPath::find($this->_path['helper'], $this->_createFileName('helper', array(
			'name' => $file
		)));
	...
	protected function _addPath($type, $path)
	{
		...
		foreach ($path as $dir)
		{
			...
			if (false !== strpos($dir, "/{$this->sunfw_template}/html/") && in_array($dir, $this->_path[$type]))
			{
				continue;
			}
			...
			// Register alternative path to look for template file.
			if ('template' == $type)
			{
				if (preg_match('#^.+[\\/]' . $this->getName() . '[\\/]tmpl[\\/]$#', str_replace('\\', '/', $dir)))
				{
					array_unshift($this->_path[$type], 
						
						// Template override directory of active template's niche style.
						JPATH_THEMES . "/{$this->sunfw_template}/niches/{$this->niche}/html/{$this->component}/" . $this->getName(), 
						
						// Template override directory in active template folder.
						JPATH_THEMES . "/{$this->sunfw_template}/html/{$this->component}/" . $this->getName(), 
						
						// Sun Framework's template override directory.
						SUNFW_PATH . "/includes/overwrite/html/{$this->component}/" . $this->getName());
				}
				elseif (preg_match('#^.+[\\/]components[\\/]' . $this->component . '[\\/](.+[\\/])$#', str_replace('\\', '/', $dir), $match))
				{
					array_unshift($this->_path[$type], 
						
						// Template override directory of active template's niche style.
						JPATH_THEMES . "/{$this->sunfw_template}/niches/{$this->niche}/html/{$this->component}/" . $match[1], 
						
						// Template override directory in active template folder.
						JPATH_THEMES . "/{$this->sunfw_template}/html/{$this->component}/" . $match[1], 
						
						// Sun Framework's template override directory.
						SUNFW_PATH . "/includes/overwrite/html/{$this->component}/" . $match[1]);
				}
			}
		}
	}

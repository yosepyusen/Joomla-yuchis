...
abstract class JModuleHelper
{
	...
	public static function renderModule($module, $attribs = array())
	{
		...
		if (!is_object($module) || !isset($module->module) || !isset($module->params))
		{
			if (JDEBUG)
			{
				JLog::addLogger(array(
					'text_file' => 'jmodulehelper.log.php'
				), JLog::ALL, array(
					'modulehelper'
				));
		...
		// Search for module chrome template file.
		foreach ($locations as $location)
		{
			$chromePath = "{$location}/modules.php";
		...
		$app->triggerEvent('onRenderModule', array(
			&$module,
			&$attribs
		));
	...
	public static function getLayoutPath($module, $layout = 'default')
	{
		...
		foreach ($locations as $location)
		{
			$tPath = "{$location}/{$module}/{$layout}.php";
	...
	protected static function &load()
	{
		...
		$app->triggerEvent('onPrepareModuleList', array(
			&$modules
		));
		...
		$app->triggerEvent('onAfterModuleList', array(
			&$modules
		));
		
		$modules = static::cleanModuleList($modules);
		
		$app->triggerEvent('onAfterCleanModuleList', array(
			&$modules
		));
		...
		switch ($cacheparams->cachemode)
		{
			case 'id':
				$ret = $cache->get(array(
					$cacheparams->class,
					$cacheparams->method
				), $cacheparams->methodparams, $cacheparams->modeparams, $wrkarounds, $wrkaroundoptions);
			break;
			...
			case 'safeuri':
				...
				$ret = $cache->get(array(
					$cacheparams->class,
					$cacheparams->method
				), $cacheparams->methodparams, $module->id . $view_levels . $secureid, $wrkarounds, $wrkaroundoptions);
			break;
			...
			case 'static':
				$ret = $cache->get(array(
					$cacheparams->class,
					$cacheparams->method
				), $cacheparams->methodparams, $module->module . md5(serialize($cacheparams->methodparams)), $wrkarounds, $wrkaroundoptions);
			break;
			...
			case 'oldstatic':
				$ret = $cache->get(array(
					$cacheparams->class,
					$cacheparams->method
				), $cacheparams->methodparams, $module->id . $view_levels, $wrkarounds, $wrkaroundoptions);
			break;
			...
			default:
				$ret = $cache->get(array(
					$cacheparams->class,
					$cacheparams->method
				), $cacheparams->methodparams, $module->id . $view_levels . JFactory::getApplication()->input->getInt('Itemid', null), $wrkarounds, 
					$wrkaroundoptions);
			break;
		}

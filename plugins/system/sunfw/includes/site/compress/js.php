 ...
 * @since       1.0.0
 */
abstract class SunFwSiteCompressJs
{
	...
	public static function compress($scripts)
	{
		...
		// Parse script tags
		foreach ($scripts as $src => $args)
		{
			// Set default group
			$attributes['group'] = 'default';
			$attributes['src'] = $src;

			// Check if script should be loaded within conditional checking tags?
			if (@is_array($args['options']) && !empty($args['options']['conditional']))
			{
			...
			if (!( $isInternal = SunFwSiteCompressHelper::isInternal($attributes['src']) ) or strpos($attributes['src'], '//') === 0)
			{
				// Add collected files to compress list
				if (!empty($groupFiles))
				{
			...
			if (( $questionPos = strpos($attributes['src'], '?') ) !== false)
			{
				...
				$path = SunFwSiteCompressHelper::getFilePath(substr($attributes['src'], 0, $questionPos));

				// Check if this is a dynamic generation content
				if (!$isDynamic and SunFwSiteCompressHelper::isInternal($attributes['src']))
				{
			...
			{
				$attributes['group'] = 'reserve|' . $attributes['src'];
			}
		...
		{
			$compressPath = JPATH_ROOT . '/' . rtrim($systemData['cacheDirectory'], '\\/');
		}
		else
		{
			$compressPath = rtrim($systemData['cacheDirectory'], '\\/');
		}
		...
		foreach ($compress as $group)
		{
			...
			if (!is_file($compressPath . $compressFile) || $modifiedFlag)
			{
				...
				$maxFileSize = $maxFileSize = 1024 * (int) $systemData['maxCompressionSize'];
				$currentSize = 0;
				...
				foreach ($group['files'] as $file)
				{
					...
					$source = str_replace(str_replace('\\', '/', JPATH_ROOT), '', str_replace('\\', '/', $filePath));
					$source = "/* FILE: {$source} */\n" . self::minify(JFile::read($filePath));
					...
					if ($length > $maxFileSize or ( $currentSize + $length ) > $maxFileSize)
					{
						if (!empty($buffer))
						{
							...
							if ($currentSize > 0)
							{
								...
								$splittedFiles[] = $prefix . str_replace(str_replace('\\', '/', JPATH_ROOT), JUri::root(true),
				...
				if (count($splittedFiles))
				{
					...
					{
						SunFwSiteCompressHelper::prependIntoFile("// Include: {$splittedFiles[$i]}" . ( $i + 1 < $n ? "\n" : "\n\n" ),
		...
		// Verify if scripts associated with this page has been changed
					...
					if (isset($tracking) && isset($tracking[$pageLink]) && isset($tracking[$pageLink]['js']))
					{
						...
						foreach ($cleanUp as $file)
						{
							...
							foreach ($tracking as $link => $assets)
							{
								...
								if (!empty($assets['js']) && in_array($file, $assets['js']))
								{
									$removable = false;
	 ...
	 * @return  string
	 */
	protected static function minify($source)
	{
		...
		// Remove all leading while-spaces.
		$source = preg_replace('#(\r?\n)[ \t]+#', '\\1', $source);

		// Remove all trailing while-spaces.
		$source = preg_replace('#[ \t]+(\r?\n)#', '\\1', $source);

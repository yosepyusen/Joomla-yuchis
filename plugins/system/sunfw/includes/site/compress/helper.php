 ...
 * @since       1.0.0
 */
abstract class SunFwSiteCompressHelper
{
	...
	public static function getFilePath($file)
	{
		...
		{
			$possiblePaths = array(
				str_replace(JUri::base(true), JPATH_ROOT, $file),
				$_SERVER['DOCUMENT_ROOT'] . '/' . ltrim($file, '/'),
	...
	public static function getRelativeFilePath($currentPath, $filePath)
	{
		...
		$realPath = realpath($currentPath . '/' . ltrim($filePath));
	...
	public static function parseAttributes($markup)
	{
		...
		{
			$attributes = array_combine(array_map('strtolower', array_map('trim', $matches[1])), $matches[3]);
		}
	...
	{
		$combine = $string . file_get_contents($filename);

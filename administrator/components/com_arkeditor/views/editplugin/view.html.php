...
class ARKViewEditPlugin extends JViewLegacy
{
	...
	{
        if($item->iscore)
           @$data = file_get_contents( JPATH_COMPONENT.'/editor/plugins/'.$item->name.'.xml' );
        else
           @$data = file_get_contents( JPATH_PLUGINS.'/arkeditor/'.$item->name.'/'.$item->name.'.xml' );
		...
		ARKForm::addFieldPath(JPATH_COMPONENT  . '/models/fields');

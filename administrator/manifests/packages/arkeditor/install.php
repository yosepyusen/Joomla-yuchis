...
class pkg_arkeditorInstallerScript
{
	...
	private $inlinefields_plugins = array
	(
		'editor',
		'textarea',
		'text',
	...
	public function preflight($type, $parent)
	{
		...
		$hideMessage = '<style>div#arkeditor-welcome { display : none; }</style>';

	    //Check system PHP version 
		if( version_compare( PHP_VERSION, '5.5.38', 'lt' ) ) 
		{
			JError::raiseWarning(null, JText::sprintf( 'JLIB_INSTALLER_MINIMUM_PHP', '5.6').$hideMessage);
			return false;
		}
		...
		{
			JError::raiseWarning(null, JText::sprintf( 'COM_ARKEDITOR_CUSTOM_INSTALL_NOT_JOOMLA_PRIOR', $this->minimum_joomla_release ). $hideMessage );
			return false;
		}
	...
	public function uninstall($parent) 
    {
		...
		$app->input->set('ark.package.uninstall',true);
	}
	...
	public function postflight($parent)
	{
		...
		foreach($this->arkeditor_plugins as $name)
		{
			...
			*/
			...
			$title				= (!empty($manifestTitle) ?  $manifestTitle : ucFirst($name));
			$jckRow->title 		= ((!empty($icon) || !empty($manifestTitle)) ? (string) $title : '');
			$jckRow->name		= $name;
			$jckRow->type 		= 'plugin';
			$jckRow->row	 	= 4;
			$jckRow->published 	= 1;
		...
		{
			$query->where('element <> '. $db->quote('arkmanager'));
		}	
		...
		//Publish inline fields plugins

		$query = $db->getQuery(true);
		$query->update('#__extensions')
			->set('enabled = 1')
			->where('folder = '.$db->quote('inlinefields'))
			->where('element IN ('. $db->quote(implode($db->quote(','),$this->inlinefields_plugins),false).')');
